...
let humanReadableToBytes str =
  try
    Scanf.sscanf str "%d kB" (fun kb -> 1000 * kb)
  with _ ->
    0
...
let parse_status raw_status_contents =
  ...
  {
    rss_anon = SMap.get "RssAnon" stats |> Option.value_map ~default:0 ~f:humanReadableToBytes;
    rss_file = SMap.get "RssFile" stats |> Option.value_map ~default:0 ~f:humanReadableToBytes;
    rss_shmem = SMap.get "RssShmem" stats |> Option.value_map ~default:0 ~f:humanReadableToBytes;
    rss_total = SMap.get "VmRSS" stats |> Option.value_map ~default:0 ~f:humanReadableToBytes;
...
let asset_procfs_supported =
  ...
  fun () ->
    ...
    | None ->
      let supported =
        if Sys.unix && Sys.file_exists "/proc"
        then Ok ()
        else Error "Proc filesystem not supported"
      in
