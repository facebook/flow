...
let cat file =
  ...
  Lwt.protected (
    ...
    | Unix.Unix_error(Unix.ENOENT, _, _) ->
      Lwt.return_error "File doesn't exist"
    | Unix.Unix_error(Unix.EBADF, _, _) ->
      Lwt.return_error "Lost fd for the file"
  )

(* I've never seen cgroup mounted elsewhere, so it's probably fine to hardcode this for now *)
let cgroup_dir = "/sys/fs/cgroup"

let assert_is_using_cgroup_v2 = Memoize.forever ~f:(fun () ->
  if%lwt Lwt_unix.file_exists cgroup_dir
  then begin
    (* /sys/fs/cgroup/memory exists for cgroup v1 but not v2. It's an easy way to tell the
     * difference between versions *)
    if%lwt Lwt_unix.file_exists (spf "%s/memory" cgroup_dir)
    then Lwt.return_error (spf "cgroup v1 is mounted at %s. We need v2" cgroup_dir)
    else Lwt.return_ok ()
  end else Lwt.return_error (spf "%s doesn't exist" cgroup_dir)
)

(* I don't really expect us to switch cgroups often, but let's only cache for 5 seconds *)
let get_cgroup_name = Memoize.until ~seconds:5.0 ~f:(fun () ->
...
type stats = {
  ...
  total_swap: int; (* The total amount of anonymous memory paged out to swap *)

  (* anon, file, and shmem are disjoint. If you add in the memory that the kernel uses, they should
   * sum roughly to `total` *)
  anon: int; (* The amount of physical anonymous memory not used for shared memory *)
  shmem: int; (* The amount of physical anonymous memory being used as shared memory *)
...
let read_single_number_file path =
  ...
  Lwt.return (
    ...
    try
      Ok (contents |> String.strip |> int_of_string)
    with
...
let (>>%) (type a) (type b) (type c)
    (thread: (a, b) result Lwt.t)
    (f: a -> (c, b) result Lwt.t)
    : (c, b) result Lwt.t =
