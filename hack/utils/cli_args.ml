...
let get_save_state_spec (v: string option) : (save_state_spec_info option, string) result =
  match v with
  | None -> Ok None
  | Some blob ->
    let open Hh_json.Access in
    ...
    | Ok ((parsed_spec: save_state_spec_info), _keytrace) ->
      ...
      Ok (Some parsed_spec)
    | Error spec_failure ->
      let message = Printf.sprintf "Parsing failed:\n%s\nSee input: %s\n"
...
let get_saved_state_spec (v: string option) : (saved_state_target_info option, string) result =
  ...
  | Some blob ->
    ...
    | Ok (parsed_data_dump, _), Error _ ->
      Ok (Some parsed_data_dump)
    | Error _, Ok (parsed_from_file, _) ->
      Ok (Some parsed_from_file)
