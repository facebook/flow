...
let get_defs ast =
  ...
  List.fold_right begin fun def (acc1, acc2, acc3, acc4 as acc) ->
    match def with
    ...
    (* namespace nodes will never appear as long as elaborate_namespaces is set
     * to true when the parser is invoked *)
    | Ast.Namespace _
