...
import flow from '../../js/initialized-flow-provider';
...
function getFlowErrors(languageClassName, code) {
  if (!jsLanguages.includes(languageClassName)) return [];
  if (!code.includes('@flow')) return [];
  return flow
    .checkContent("example.js", code)
    .flatMap(({message}) => message)
    .map(
      ({loc,descr}) => `${loc.start.line}:${loc.start.column}-${loc.end.line}:${loc.end.column}: ${descr}`
    );
}
...
export default function CodeBlock({
  ...
  const flowErrors = getFlowErrors(languageClassName, children);
  ...
  const isJSLanguage = !jsLanguages.includes(languageClassName);
          ...
          <div className={clsx(styles.codeBlockContent, language)}>
            ...
            {flowErrors.length > 0 && (
              <pre className={styles.flowErrors}>
                {flowErrors.map((error, index) => (<div key={index}>{error}</div>))}
              </pre>
            )}
          </div>
        </div>
