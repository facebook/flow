...
let recheck_start ~start_state =
  internal_state.last_recheck_start_state <- Some start_state
...
let log_pushed_errors ~end_state ~errors_reason =
  ...
  in
  List.iter triggers ~f:(fun trigger ->
...
let rec gc ~get_state oldest_allowed =
  ...
  then begin
    ...
    | Some interaction ->
      ...
      else Some interaction.start_state.time
  end
  else None
