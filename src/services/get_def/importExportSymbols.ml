...
let find_related_symbol_from_module_kind loc = function
  ...
  | ES { named; _ } ->
    ...
    Base.List.find_map ~f:(find_related_symbol_from_export loc) exports
  ...
  | BindNamed named ->
    let loc_records (* list of related loc *) =
      List.fold_left
        ...
        named
    in
    Base.List.find_map ~f:(fun x -> x) loc_records
...
let find_related_symbol_from_requires loc requires =
  Base.List.find_map ~f:(find_related_symbol_from_require loc) requires
