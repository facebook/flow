  ...
  class searcher ~(covers_target : ALoc.t -> bool) =
    object (this)
      ...
      method! import_named_specifier ~import_kind:_ specifier =
        let open Flow_ast.Statement.ImportDeclaration in
        let { kind = _; kind_loc = _; local; remote; remote_name_def_loc = _ } = specifier in
...
let get_def_info ~loc_of_aloc ~purpose (ast, file_sig, _) type_info loc : (def_info, string) result
    =
