...
module FlowProgram : sig
  ...
  let init ~focus_targets genv =
    ...
    Hh_logger.info "executable=%s" (executable_path ());
...
let rec recheck_loop ~dfind genv env =
  ...
  if SSet.is_empty raw_updates then env else begin
    let updates = FlowProgram.process_updates genv env raw_updates in
    let env = FlowProgram.recheck genv env updates in
...
let process_message genv env request =
  ...
  | MonitorProt.Request (request_id, command) ->
    FlowProgram.handle_command genv env (request_id, command)
  ...
  | MonitorProt.PersistentConnectionRequest (client_id, request) ->
    FlowProgram.handle_persistent_client genv env client_id request
...
let init_dfind options =
  ...
  let watch_paths = FlowProgram.get_watch_paths options in
...
let create_program_init ~shared_mem_config ~focus_targets options =
  ...
  let program_init = fun () ->
    let profiling, env = FlowProgram.init ~focus_targets genv in
...
let check_once ~shared_mem_config ~client_include_warnings ?focus_targets options =
  ...
  let errors, warnings, suppressed_errors = FlowProgram.check_once genv env in
