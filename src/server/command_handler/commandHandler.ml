    ...
    | Ok check_result ->
      ...
      let json =
        ...
        let json_props =
          match result with
          | Ok [] ->
            (match File_content.get_offset content { File_content.line; column = col } with
            (* In case it fails, we should not crash trying to generate good logs. *)
            | exception Invalid_argument _ -> ("broader_context", Hh_json.JSON_Null) :: json_props
            | offset ->
              ...
              let after =
                String.sub content offset (Int.min 100 (String.length content - offset))
              in
  ...
  | Ok (filename, contents) ->
    ...
    let (ast_opt, has_parse_errors) =
      ...
      | None ->
        ...
        in
        let has_parse_errors = not (Flow_error.ErrorSet.is_empty parse_errors) in
        begin
          match parse_result with
          | None -> (None, has_parse_errors)
          | Some (Parse_artifacts { ast; _ }) -> (Some ast, has_parse_errors)
        end
    in
...
let vscode_detailed_diagnostics client =
