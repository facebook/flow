...
module ServerMain (Program : SERVER_PROGRAM) : sig
  ...
  let sleep_and_check socket persistent_connections =
    ...
    match ready_socket_l with
      | [] -> None
      | ready_socket::_ ->
          if ready_socket = socket then
            Some (New_client socket)
  ...
  let respond_to_client env client =
    let msg, env =
      try
        `Msg (Persistent_connection.input_value client), env
      with
        | End_of_file ->
            print_endline "Lost connection to client";
            let new_connections = Persistent_connection.remove_client env.connections client in
            `Eof, {env with connections = new_connections}
    in
    match msg with
      | `Msg Persistent_connection_prot.Subscribe ->
          let new_connections =
  ...
  let serve genv env socket =
    ...
    while true do
      ...
      let ready_socket = sleep_and_check socket !env.connections in
