        ...
        let have_subtyping_errors_in_file =
          ConcreteLocPrintableErrorSet.exists
            (fun error ->
              ...
              | Some
                  Error_codes.(
                    ( IncompatibleExact | IncompatibleFunctionIndexer | IncompatibleIndexer
                    | IncompatibleType | IncompatibleTypeGuard | IncompatibleUse
                    | IncompatibleVariance | PropMissing )) ->
                true
              | _ -> false)
            errors
        in
