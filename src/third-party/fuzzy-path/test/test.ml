...
let tests =
  ...
  >::: [
         ( "can_match_strings" >:: fun ctxt ->
           ...
           assert_values ~ctxt ["abC"; "abcd"; "AlphaBetaCappa"; "alphabetacappa"] result;

           let result = Fuzzy_path.search "t/i/a/t/d" matcher in
           assert_values ~ctxt ["/this/is/a/test/dir"] result
         );
         ( "prefers_exact_over_abbr_over_others" >:: fun ctxt ->
           ...
           assert_values
             ~ctxt
             ["/test/tiatd"; "/this/is/a/test/dir"; "/////ThisIsATestDir"; "thisisatestdir"]
             result
         );
         ( "defaults_case_insensitive" >:: fun ctxt ->
           ...
           assert_values ~ctxt ["AlphaBetaCappa"; "abC"; "abcd"; "alphabetacappa"] result
         );
         ( "ignores_spaces" >:: fun ctxt ->
           let matcher = Fuzzy_path.init candidates in
           let result = Fuzzy_path.search "a b\tcappa" matcher in
           assert_values ~ctxt ["AlphaBetaCappa"; "alphabetacappa"] result
         );
