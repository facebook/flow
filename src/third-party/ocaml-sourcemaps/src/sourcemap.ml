...
let create ?file ?source_root () =
  { file; source_root; names = SSet.empty; mappings = Addition []; sources_contents = SMap.empty }
...
module MappingHolder = struct
  ...
  let make_lookup mapping_arr = Lookup { mapping_arr; last_ind = -1 }
...
let rec binary_search ~cmp needle arr l u =
  ...
  if len = 0 then
    ...
    if k = 0 then
      ...
      binary_search ~cmp needle arr l (i - 1)
    else
      binary_search ~cmp needle arr (i + 1) u
...
let find_original_unknown_arr mappings_arr generated =
  ...
  let cmp { line = a_line; col = a_col } { generated_loc = { line = b_line; col = b_col }; _ } =
...
let find_original_from_context c generated =
  ...
  if len = 0 then
    ...
    let try_local_search =
      if c.last_ind < 0 then
        None
      else
        let recent_line = line_for_index c.last_ind in
        if generated.line <> recent_line then
          None
        else
          ...
          let rec look_right cur_ind looks_remaining =
            if cur_ind = len - 1 || line_for_index (cur_ind + 1) <> recent_line then
              Some (c.mapping_arr.(cur_ind), cur_ind)
            else if column_for_index (cur_ind + 1) <= generated.col then
              Some (c.mapping_arr.(cur_ind + 1), cur_ind + 1)
            else if looks_remaining = 0 then
              None
            else
              look_right (cur_ind + 1) (looks_remaining - 1)
          in
          let rec look_left cur_ind looks_remaining =
            ...
            then
              ...
              look_left (cur_ind - 1) (looks_remaining - 1)
          in
...
let compose map map2 =
  let (mappings, names, sources_contents) =
    List.fold_left
      (fun (mappings, names, sources_contents) mapping ->
        match mapping.original with
        | Some { original_loc; _ } ->
          begin
            match find_original map2 original_loc with
            | Some ({ name; source; _ } as original) ->
              let mapping = { mapping with original = Some original } in
              let names =
                match name with
                | Some name -> SSet.add name names
                | None -> names
              in
        ...
        | None -> (mappings, names, sources_contents))
      ([], SSet.empty, SMap.empty)
      (MappingHolder.addition_list map.mappings)
  in
  {
    map with
    mappings = Addition (List.rev mappings);
    names;
    sources_contents;
...
let add_mapping ~original ~generated map =
  let names =
    ...
    | None -> map.names
  in
  let mapping = { original = Some original; generated_loc = generated } in
...
let index_map set =
  let (_, map) =
    SSet.fold (fun item (i, map) -> (i + 1, SMap.add item i map)) set (0, SMap.empty)
  in
  map
...
let string_of_mappings map =
  ...
  let (buf, _) =
    List.fold_left
        ...
        let state =
          ...
          let (skip, state) =
            if line_diff > 0 then begin
              ...
              (false, { state with prev_gen_col = 0; prev_gen_line = mapping.generated_loc.line })
            end else
              let skip =
                match state.prev_mapping with
                | Some prev_mapping ->
                  begin
                    match (prev_mapping.original, mapping.original) with
      ...
      (buf, state)
      (List.rev (MappingHolder.addition_list map.mappings))
  in
  Buffer.contents buf
...
let mappings_of_stream =
  ...
  let rec helper acc state stream sources names =
    ...
    | Some ';' ->
      (* newline *)
      let state = { state with prev_gen_line = state.prev_gen_line + 1; prev_gen_col = 0 } in
      Stream.junk stream;
    ...
    | Some _ ->
      ...
      let generated_loc = { line = state.prev_gen_line; col = offset + state.prev_gen_col } in
      let state = { state with prev_gen_col = generated_loc.col } in

      let (state, original) =
        if next_is_separator stream then
          (state, None)
        else
          ...
          let original_loc =
            { line = line_offset + state.prev_orig_line; col = col_offset + state.prev_orig_col }
          in
          let (name_idx, name) =
            if next_is_separator stream then
              (state.prev_name, None)
            else
...
let sources_contents map =
  sources map
  |> List.map (fun source ->
         if SMap.mem source map.sources_contents then
           Some (SMap.find source map.sources_contents)
         else
           None
     )
...
module Make_json_writer (Json : Json_writer_intf) : Json_writer with type json = Json.t = struct
  ...
  let json_of_sources_contents map =
    ...
    |> List.map (function
           | Some content -> Json.of_string content
           | None -> Json.null
           )
    |> Json.of_array
  ...
  let json_of_sourcemap map =
    ...
    in
    let rev_props =
      match source_root map with
      | Some root -> ("sourceRoot", Json.of_string root) :: rev_props
      | None -> rev_props
    in
    let rev_props =
      if SMap.is_empty map.sources_contents then
        rev_props
      else
        ("sourcesContent", json_of_sources_contents map) :: rev_props
    in
...
module Make_json_reader (Json : Json_reader_intf) : Json_reader with type json = Json.t = struct
  ...
  let sources_list_of_json json = List.map Json.to_string (Json.to_array json)
  let names_list_of_json json = List.map Json.to_string (Json.to_array json)
  ...
  let sources_contents_of_json sources json =
    let contents =
      Json.to_array json
      |> List.map (fun x ->
             if Json.is_null x then
               None
             else
               Some (Json.to_string x)
         )
    in
    List.fold_left2
      (fun acc source content ->
        match content with
