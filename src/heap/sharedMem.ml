...
module NewAPI = struct
  ...
  let prepare_write_addr_tbl prep xs =
    let n = Array.length xs in
    if n = 0 then
      prepare_const null_addr
    else
      let tbl_size = addr_size * n in
      let (elems_size, writes) =
        let f size_acc x =
          let (size, write) = prep x in
          (size_acc + size, write)
        in
        Array.fold_left_map f 0 xs
      in
      let write chunk =
  ...
  let haste_module_size = 5 * addr_size
  ...
  let haste_namespace_bitset_addr m = addr_offset m 2

  let haste_provider_addr m = addr_offset m 3

  let haste_all_providers_addr m = addr_offset m 4

  let haste_dependents_addr m = addr_offset m 5
