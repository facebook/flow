...
let multi_threaded_call_with_worker_id
    (type a b c)
    workers
  ...
  *)
  ...
  let rec run_worker worker =
    ...
    | Some bucket ->
      ...
      let%lwt () =
        match%lwt WorkerController.call worker (fun xl -> job ~worker_id xl) bucket with
        | None -> Lwt.return_unit
        | Some result -> merge_with_acc result
      in
      (* Wait means "ask again after a worker has finished and has merged its result". So now that
...
let call_with_worker_id workers ~job ~merge ~neutral ~next =
  if !is_busy then
    ...
    is_busy := true;
    (match workers with
    | None -> Lwt.return (single_threaded_call_with_worker_id (job ~worker_id:1) merge neutral next)
    | Some workers -> multi_threaded_call_with_worker_id workers job merge neutral next)
      [%lwt.finally
        is_busy := false;
        Lwt.return_unit]
  )
