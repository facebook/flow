...
let expect_compact effort expected =
  ...
  expect
    ~msg:
      (Printf.sprintf
         "Expected %s collection to be %sneeded"
         (match effort with
         | `gentle -> "gentle"
...
let test_gc_collect () =
  ...
  (* no memory is wasted *)
  expect_compact `gentle false;
  expect_compact `aggressive false;
  expect_heap_size 2;
  expect_mem "0";
  expect_mem "1";
  remove "1";
...
let test_gc_aggressive () =
  ...
  expect_compact `aggressive true;
  expect_heap_size 2
