...
module CallFunctor (Caller : CALLER) : sig
  ...
  let call workers ~job ~merge ~neutral ~next =
    match workers with
    | None -> Caller.return (single_threaded_call_with_worker_id job merge neutral next)
...
let call_with_interrupt ?on_cancelled workers ~job ~merge ~neutral ~next ~interrupt =
  ...
  | Some workers ->
    MultiThreadedCall.call_with_interrupt ?on_cancelled workers job merge neutral next interrupt
