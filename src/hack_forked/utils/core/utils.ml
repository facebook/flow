...
let is_prefix_dir dir fn =
  ...
  String.length fn > String.length prefix && String.sub fn 0 (String.length prefix) = prefix
...
let split_ns_from_name (s : string) : string * string =
  try
    let base_name_start = String.rindex s '\\' + 1 in
    let name_part = String.sub s base_name_start (String.length s - base_name_start) in
...
let infimum (arr : 'a array) (bound : 'b) (compare : 'a -> 'b -> int) : int option =
