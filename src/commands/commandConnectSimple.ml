...
let verify_handshake ~client_handshake ~server_handshake sockaddr ic =
  ...
  begin match server1 with
    | {server_intent = Server_will_continue; _} ->
      ()
    | {server_intent = Server_will_hangup; _} ->
      close_connection sockaddr;
  ...
  | ({server_intent = Server_will_continue; _}, Some Server_ready) ->
    Ok ()
  | ({server_intent = Server_will_continue; _}, Some (Server_still_initializing _)) ->
    Ok ()
...
let connect_once ~client_handshake ~tmp_dir root =
  ...
  try
    Timeout.with_timeout
      ...
      verify_handshake ~client_handshake ~server_handshake sockaddr ic >>= fun () ->
      Ok (ic, oc)
  with
