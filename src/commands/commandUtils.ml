...
let make_options
  ...
  let opt_log_file = server_log_file ~flowconfig_name ~tmp_dir:opt_temp_dir root in
  {
    ...
    opt_natural_inference_object_literal_followup_fix =
      FlowConfig.natural_inference_object_literal_followup_fix flowconfig;
    opt_natural_inference_object_literal_followup_fix_excludes =
      Base.List.map
        ~f:(fun pattern -> pattern |> Files.expand_project_root_token ~root |> Str.regexp)
        (FlowConfig.natural_inference_object_literal_followup_fix_excludes flowconfig);
    ...
    opt_only_support_flow_fixme_and_expected_error =
      FlowConfig.only_support_flow_fixme_and_expected_error flowconfig;
