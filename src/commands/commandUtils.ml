...
module Options_flags = struct
  type t = {
    ...
    incremental_error_collation: bool option;
...
let options_flags =
  ...
  fun prev ->
    CommandSpec.ArgSpec.(
      ...
      |> flag "--incremental-error-collation" (optional bool) ~doc:""
      |> warning_flags
      |> flowconfig_flags
      |> verbose_flags
...
let make_options
  ...
  let opt_log_file = server_log_file ~flowconfig_name ~tmp_dir:opt_temp_dir root in
  {
    ...
    opt_incremental_error_collation =
      Base.Option.value
        options_flags.incremental_error_collation
        ~default:(FlowConfig.incremental_error_collation flowconfig);
