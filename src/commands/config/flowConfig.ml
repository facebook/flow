...
module Opts = struct
  ...
  type t = {
    ...
    async_component_syntax_includes: string list;
  ...
  let use_unknown_in_catch_variables_parser =
    boolean (fun opts v -> Ok { opts with use_unknown_in_catch_variables = Some v })
  ...
  let parsers =
    [
      ...
      ( "experimental.async_component_syntax",
        boolean (fun opts v -> Ok { opts with async_component_syntax = v })
      );
      ( "experimental.async_component_syntax.includes",
        string
          ~init:(fun opts -> { opts with async_component_syntax_includes = [] })
          ~multiple:true
          (fun opts v ->
            Ok
              {
                opts with
                async_component_syntax_includes = v :: opts.async_component_syntax_includes;
              })
      );
...
let async_component_syntax_includes c = c.options.Opts.async_component_syntax_includes
...
let records_includes c = c.options.Opts.records_includes
...
let saved_state_reinit_on_lib_change c = c.options.Opts.saved_state_reinit_on_lib_change
...
let use_unknown_in_catch_variables c = c.options.Opts.use_unknown_in_catch_variables
