...
let main base_flags option_values json pretty root strip_root path wait_for_recheck expand_aliases evaluate_destructors filename () =
  ...
  let request = ServerProt.Request.DUMP_TYPES { input = file; expand_aliases; evaluate_destructors; wait_for_recheck; } in
