...
module Fix_errors_command = struct
  ...
  let doc = "Apply all known error fixes across files"
  ...
  let spec =
    {
      ...
      usage =
        Printf.sprintf
          "Usage: %s codemod fix-errors [OPTION]... [FILE]\n\n%s. For each error that has a single\nautomatic fix (via Code_action_service), applies the fix.\n"
          Utils_js.exe_name
          doc;
      args =
        CommandSpec.ArgSpec.(
  ...
  let spec =
    CommandUtils.subcommand_spec
      ...
      [
        ...
        (Fix_errors_command.spec.CommandSpec.name, Fix_errors_command.command);
