...
let rec search (arr : _ array) (start : int) (finish : int) target =
  if start > finish then false
    ...
    let a, b = arr.!(mid) in
    if target < a then search arr start (mid - 1) target
    else if target >= b then search arr (mid + 1) finish target
    else true

let is_valid_unicode_id (i : int) =
  search Js_id_unicode.id_continue 0
    (Array.length Js_id_unicode.id_continue - 1)
    i
