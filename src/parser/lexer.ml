...
let scinumber = [%sedlex.regexp?
  Star ('0'..'9'), Opt '.', Plus ('0'..'9'),
  ('e' | 'E'), Opt ('-' | '+'), Plus ('0'..'9')
]
let wholenumber = [%sedlex.regexp? Plus ('0'..'9'), Opt '.']
let floatnumber = [%sedlex.regexp? Star ('0'..'9'), '.', Plus ('0'..'9')]
...
let rec token (env: Lex_env.t) lexbuf =
  ...
  | (scinumber | wholenumber | floatnumber), word ->
    (* Numbers cannot be immediately followed by words *)
    let env = illegal env (loc_of_lexbuf env lexbuf) in
    Sedlexing.rollback lexbuf;
    let () = [%sedlex match lexbuf with
...
and type_token env lexbuf =
  ...
  | Opt neg, binnumber, (letter | '2'..'9'), Star alphanumeric ->
    ...
    let env = illegal env (loc_of_lexbuf env lexbuf) in
    Sedlexing.rollback lexbuf;
    let token = [%sedlex match lexbuf with
    | Opt neg, binnumber ->
      let num = Sedlexing.Utf8.lexeme lexbuf in
      mk_num_singleton BINARY num
    | _ -> failwith "unreachable"
    ] in
    env, token
  ...
  | Opt neg, legacyoctnumber, (letter | '8'..'9'), Star alphanumeric ->
    ...
    | Opt neg, legacyoctnumber ->
      let num = Sedlexing.Utf8.lexeme lexbuf in
      mk_num_singleton LEGACY_OCTAL num
    | _ -> failwith "unreachable"
    ] in
    env, token
  ...
  | Opt neg, hexnumber, non_hex_letter, Star alphanumeric ->
    ...
    let env = illegal env (loc_of_lexbuf env lexbuf) in
    Sedlexing.rollback lexbuf;
    let env, token = [%sedlex match lexbuf with
    | Opt neg, hexnumber ->
      let num = Sedlexing.Utf8.lexeme lexbuf in
  ...
  | Opt neg, (scinumber | wholenumber | floatnumber), word ->
    (* Numbers cannot be immediately followed by words *)
    let env = illegal env (loc_of_lexbuf env lexbuf) in
    Sedlexing.rollback lexbuf;
    let token = [%sedlex match lexbuf with
    | Opt neg, scinumber | wholenumber | floatnumber ->
      let num = Sedlexing.Utf8.lexeme lexbuf in
      mk_num_singleton NORMAL num
    | _ -> failwith "unreachable"
    ] in
    env, token
...
and jsx_tag env lexbuf =
  ...
  | '.' -> env, T_PERIOD
  | '=' -> env, T_ASSIGN
  | letter, Star ('-' | alphanumeric) -> env, T_JSX_IDENTIFIER
