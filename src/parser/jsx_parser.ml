...
module JSX (Parse: Parser_common.PARSER) = struct
  ...
  let name =
    let rec member_expression env member =
      ...
      | T_PERIOD ->
          ...
          let member = with_loc ~start_loc (fun env ->
            Expect.token env T_PERIOD;
            let property = identifier env in
            { JSX.MemberExpression.
              _object = JSX.MemberExpression.MemberExpression member;
              property;
            }
          ) env in
    ...
    in fun env ->
      ...
      | T_COLON ->
          let namespaced_name = with_loc (fun env ->
  ...
  let attribute env = with_loc (fun env ->
    let name =
      match Peek.ith_token ~i:1 env with
      | T_COLON ->
        let namespaced_name = with_loc (fun env ->
    ...
    let value =
      ...
      | T_ASSIGN ->
        ...
        | T_JSX_TEXT (loc, value, raw) as token ->
            ...
            Some (JSX.Attribute.Literal (loc, { Ast.Literal.
              value;
              raw;
    ...
    let opening_element =
      ...
      in fun env -> with_loc (fun env ->
        ...
        let element = match Peek.token env with
        | T_GREATER_THAN ->
          `Fragment
        | _ ->
    ...
    let closing_element env = with_loc (fun env ->
      Expect.token env T_LESS_THAN;
    ...
    and element =
      ...
      in let is_self_closing = function
      ...
      in fun env ->
        ...
        let result = match openingElement with
        | start_loc, `Element e ->
          `Element JSX.({
