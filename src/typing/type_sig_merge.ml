...
module type S = sig
  ...
  val merge_remote_ref : file -> Reason.reason -> ALoc.t Pack.remote_ref -> Type.t
  ...
  val merge_exports : file -> Reason.reason -> exports -> Type.t
...
module Make (ConsGen : CONS_GEN) : S = struct
  ...
  let merge_def file reason = function
    | TypeAlias { id_loc = _; name; tparams; body } ->
      merge_type_alias file reason name tparams body
