...
= struct
  ...
  class ssa_builder = object(this)
    ...
    method values: Ssa_api.values =
      L.LMap.map Val.simplify values
    ...
    method any_identifier (loc: L.t) (x: string) =
      begin match SMap.get x ssa_env with
        | Some { val_ref; _ } ->
          values <- L.LMap.add loc !val_ref values
        | None -> ()
      end;
    ...
    (* Order of evaluation matters *)
    method! variable_declarator ~kind (decl: (L.t, L.t) Ast.Statement.VariableDeclaration.Declarator.t) =
    ...
    method private ssa_switch_case (env, case_completion_states) (case: (L.t, L.t) Ast.Statement.Switch.Case.t') =
...
module With_Loc = Make
    (Loc_sig.LocS)
    (Ssa_api.With_Loc)
    (Scope_builder.With_Loc)

module With_ALoc = Make
    (Loc_sig.ALocS)
    (Ssa_api.With_ALoc)
