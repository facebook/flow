  ...
  let rec __flow cx ((l : Type.t), (u : Type.use_t)) trace =
    ...
    **)
       ...
       unsealed rest result object. For now, the design here is incomplete. *)
        ...
        | (AnyT (_, src), ObjSealT (reason, tout)) -> rec_flow_t cx trace (AnyT.why src reason, tout)
        (*************************)
    ...
    | None ->
      ...
      | Named _ ->
        ...
        rec_flow
          cx
          trace
          (o.proto_t, LookupT (reason_op, strict, [], propref, ReadProp { use_op; obj_t = l; tout }))
      | Computed elem_t ->
        (match elem_t with
        | OpenT _ ->
          let loc = loc_of_t elem_t in
...
*)
  ...
  and lookup_builtin cx ?trace x reason strict builtin =
    ...
    flow_opt
      ...
      ( l,
        LookupT
          (reason, strict, [], propref, ReadProp { use_op = unknown_use; obj_t = l; tout = builtin })
      )
