...
module M__flow
  ...
  (AssertGround: Assert_ground.ASSERT_GROUND)
= struct
...
let rec __flow cx ((l: Type.t), (u: Type.use_t)) trace =
  if ground_subtype (l, u) then
    ...
    *)
    ...
    | DefT (_, AnyT _), GetProtoT (reason_op, t) ->
    ...
    **)
    ...
    | DefT (_, AnyT _), MatchPropT (use_op, reason_op, _, t) ->
        rec_flow cx trace (t, UseT (use_op, AnyT.untyped reason_op))
...
module rec FlowJs: Flow_common.S = struct
  ...
  include M__flow (React) (AssertGround)
