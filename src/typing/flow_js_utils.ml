...
module TypeAssertions : sig
  val run : Context.t -> reason -> Type.type_assertion_kind -> Type.t -> unit
end = struct
  let assert_binary_in_lhs cx = function
  ...
  let assert_binary_in_rhs cx = function
  ...
  let assert_for_in_rhs cx = function
    | l when object_like l -> ()
    | AnyT _
    | ObjProtoT _ ->
      ()
  ...
  let assert_non_component_like cx def_loc use_reason = function
    ...
    | DefT
        (reason_type, (ObjT { call_t = Some _; _ } | FunT _ | ClassT _ | ReactAbstractComponentT _))
      ->
      add_output
        cx
        Error_message.(
  ...
  let assert_instanceof_rhs cx = function
    (********************)
    (* `instanceof` RHS *)
  ...
  let run cx use_reason kind t =
    match kind with
    | TypeAssertionBinaryInLHS -> assert_binary_in_lhs cx t
    | TypeAssertionBinaryInRHS -> assert_binary_in_rhs cx t
    | TypeAssertionForInRHS -> assert_for_in_rhs cx t
    | TypeAssertionInstanceofRHS -> assert_instanceof_rhs cx t
    | TypeAssertionNonComponentLike def_loc -> assert_non_component_like cx def_loc use_reason t
end
