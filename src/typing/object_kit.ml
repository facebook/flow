...
module type OBJECT = sig
  val run: Context.t -> Trace.t -> use_op:Type.use_op -> Reason.t -> Type.Object.resolve_tool ->
    Type.Object.tool -> Type.t -> Type.t -> unit
end
...
let run =
  ...
  let read_prop r flags x p =
    ...
    | Some t -> t
    | None ->
      let reason = replace_reason_const ~keep_def_loc:true (RUnknownProperty (Some x)) r in
  ...
  let read_dict r {value; dict_polarity; _} =
    ...
    then value
      ...
      let reason = replace_reason_const ~keep_def_loc:true (RUnknownProperty None) r in
  ...
  let object_rest =
    ...
    let rest cx trace ~use_op reason merge_mode
      ...
      let props = SMap.merge (fun k p1 p2 ->
        ...
        | (Sound | IgnoreExactAndOwn),
          None, Some (t2, _), _ ->
          let reason = replace_reason_const ~keep_def_loc:true (RUndefinedProperty k) r1 in
        ...
        | ReactConfigMerge _,
          ...
          if flags1.exact then (
            ...
            let r2 = replace_reason_const ~keep_def_loc:true (RProperty (Some k)) r2 in
