...
let not_string_literal expected = function
  | DefT (r, trust, StrT (Literal (_, actual))) when actual = expected -> DefT (r, trust, EmptyT Bottom)
  | t -> t
...
let boolean t =
  match t with
  | DefT (r, trust, MixedT Mixed_truthy) -> DefT (replace_desc_new_reason BoolT.desc r, trust, BoolT (Some true))
...
let string t =
  match t with
  | DefT (r, trust, MixedT Mixed_truthy) -> DefT (replace_desc_new_reason StrT.desc r, trust, StrT Truthy)
...
let number t =
  match t with
  | DefT (r, trust, MixedT Mixed_truthy) -> DefT (replace_desc_new_reason NumT.desc r, trust, NumT Truthy)
...
let array t =
  match t with
  | DefT (r, trust, MixedT _) ->
    DefT (replace_desc_new_reason RROArrayType r, trust,
      ArrT (ROArrayAT (DefT (r, trust, MixedT Mixed_everything)))
    )
  | DefT (_, _, (ArrT _)) | AnyT _ ->
    t
  | _ ->
    DefT (reason_of_t t, bogus_trust (), EmptyT Bottom)
