    ...
    *)
        ...
        Generic
          {
            Generic.id =
              Generic.Identifier.Unqualified
                (name_loc, ({ Ast.Identifier.name; comments } as id_name));
            targs;
          } ) as t_ast ->
        ...
        | "$PropertyType" ->
          check_type_arg_arity cx loc t_ast targs 2 (fun () ->
              ...
              | ([t; DefT (_, _, SingletonStrT key)], targs) ->
                ...
                reconstruct_ast
                  (EvalT (t, TypeDestructorT (use_op reason, reason, PropertyType key), mk_id ()))
                  targs
     ...
     type T *)
        | "$ElementType" ->
          check_type_arg_arity cx loc t_ast targs 2 (fun () ->
              ...
              | ([t; e], targs) ->
                ...
                reconstruct_ast
                  (EvalT (t, TypeDestructorT (use_op reason, reason, ElementType e), mk_id ()))
                  targs
              | _ -> assert false)
        ...
        | "$NonMaybeType" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              ...
              reconstruct_ast
                (EvalT (t, TypeDestructorT (use_op reason, reason, NonMaybeType), mk_id ()))
                targs)
        ...
        | "$ReadOnly" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              ...
              reconstruct_ast
                (EvalT (t, TypeDestructorT (use_op reason, reason, ReadOnlyType), mk_id ()))
                targs)
        ...
        | "$Values" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              ...
              reconstruct_ast
                (EvalT (t, TypeDestructorT (use_op reason, reason, ValuesType), mk_id ()))
                targs)
        | "$Exact" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              let (ts, targs) = convert_type_params () in
        ...
        | "$TupleMap" ->
          check_type_arg_arity cx loc t_ast targs 2 (fun () ->
              ...
              reconstruct_ast
                (EvalT
                   (t1, TypeDestructorT (use_op reason, reason, TypeMap (TupleMap t2)), mk_id ()))
                targs)
        | "$ObjMap" ->
          check_type_arg_arity cx loc t_ast targs 2 (fun () ->
              let (t1, t2, targs) =
              ...
              reconstruct_ast
                (EvalT
                   (t1, TypeDestructorT (use_op reason, reason, TypeMap (ObjectMap t2)), mk_id ()))
                targs)
        | "$ObjMapi" ->
          check_type_arg_arity cx loc t_ast targs 2 (fun () ->
              let (t1, t2, targs) =
              ...
              reconstruct_ast
                (EvalT
                   (t1, TypeDestructorT (use_op reason, reason, TypeMap (ObjectMapi t2)), mk_id ()))
                targs)
        | "$CharSet" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              match targs with
        ...
        | "React$ElementProps" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              ...
              reconstruct_ast
                (EvalT (t, TypeDestructorT (use_op reason, reason, ReactElementPropsType), mk_id ()))
                targs)
        | "React$ElementConfig" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              let (ts, targs) = convert_type_params () in
              ...
              reconstruct_ast
                (EvalT
                   (t, TypeDestructorT (use_op reason, reason, ReactElementConfigType), mk_id ()))
                targs)
        | "React$ElementRef" ->
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              let (ts, targs) = convert_type_params () in
              ...
              reconstruct_ast
                (EvalT (t, TypeDestructorT (use_op reason, reason, ReactElementRefType), mk_id ()))
                targs)
        ...
        | "$Refine" ->
          check_type_arg_arity cx loc t_ast targs 3 (fun () ->
              ...
              | ([base_t; fun_pred_t; DefT (_, _, SingletonNumT (f, _))], targs) ->
                ...
                reconstruct_ast (EvalT (base_t, LatentPredT (reason, pred), mk_id ())) targs
        ...
        Function
          {
            Function.params =
              ( params_loc,
                { Function.Params.params = List.rev rev_param_asts; rest = rest_param_ast } );
            return = return_ast;
            tparams = tparams_ast;
          } )
    ...
    | (loc, Interface { Interface.extends; body }) ->
      ...
      let (iface_sig, property_asts) =
        add_interface_properties cx tparams_map properties iface_sig
      in
      ...
      ( (loc, Class_type_sig.thistype cx iface_sig),
        Interface
          {
            Interface.body =
              (body_loc, { Object.exact; inexact = false; properties = property_asts });
            extends = extend_asts;
          } )
    | (loc, Exists) ->
...
and convert_object =
  ...
  Ast.Type.(
    ...
    let make_dict cx tparams_map indexer =
      ...
      ( {
          Type.dict_name = Option.map ~f:ident_name id;
          key;
          value;
...
and mk_func_sig =
  Ast.Type.Function.(
    ...
    let add_rest cx tparams_map x rest_param =
      let (rest_loc, { RestParam.argument = (loc, { Param.name; annot; optional }) }) =
        rest_param
      in
      let (((_, t), _) as annot) = convert cx tparams_map annot in
      let name = Option.map ~f:(fun (loc, id_name) -> ((loc, t), id_name)) name in
    ...
    let convert_params cx tparams_map (loc, { Params.params; rest }) =
      let fparams =
        Func_type_params.empty (fun params rest -> Some (loc, { Params.params; rest }))
      in
