...
module Make (Statement : Statement_sig.S) : Type_annotation_sig.S = struct
  ...
  let polarity cx variance =
    ( if not (Context.ts_syntax cx) then
      ...
      | Some (loc, { Ast.Variance.kind = Ast.Variance.Readonly; _ })
        when not (Context.allow_readonly_variance cx) ->
  ...
  let resolve_computed_key_name cx expr =
    match Flow_ast_utils.well_known_symbol_name expr with
    | Some name ->
      let typed_expr = Statement.expression cx expr in
      (typed_expr, Some (OrdinaryName name))
    | None ->
      let (((_, key_t), _) as typed_expr) = Statement.expression ~as_const:true cx expr in
      let reason = TypeUtil.reason_of_t key_t in
      let concrete_keys = Flow.possible_concrete_types_for_computed_object_keys cx reason key_t in
      let resolved_name =
        match concrete_keys with
        | [key] ->
  ...
  let rec convert env =
    ...
    | ( loc,
      ...
      begin
        ...
        | "$NonMaybeType" ->
          if Context.is_utility_type_deprecated cx "$NonMaybeType" then begin
            Flow_js_utils.add_output
              cx
              (Error_message.EIncorrectTypeWithReplacement
                 { loc; kind = Flow_intermediate_error_types.IncorrectType.DollarNonMaybeType }
              )
          end;
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              let (ts, targs) = convert_type_params () in
        ...
        (* $ReadOnly<T> *)
        | "$ReadOnly" ->
          if Context.is_utility_type_deprecated cx "$ReadOnly" then begin
            Flow_js_utils.add_output
              cx
              (Error_message.EIncorrectTypeWithReplacement
                 { loc; kind = Flow_intermediate_error_types.IncorrectType.DollarReadOnly }
              )
          end;
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              let (ts, targs) = convert_type_params () in
        ...
        | "$Values" ->
          if Context.is_utility_type_deprecated cx "$Values" then begin
            Flow_js_utils.add_output
              cx
              (Error_message.EIncorrectTypeWithReplacement
                 { loc; kind = Flow_intermediate_error_types.IncorrectType.DollarValues }
              )
          end;
          check_type_arg_arity cx loc t_ast targs 1 (fun () ->
              let (ts, targs) = convert_type_params () in
        ...
        if optional && _method && not (Context.tslib_syntax env.cx) then (
          ...
          | Ast.Expression.Object.Property.Computed
            ...
            | None ->
              ...
              let (_, prop_ast) =
                Tast_utils.error_mapper#object_property_type (computed_loc, prop)
              in
              (acc, prop_ast))
      ...
      ( if Subst_name.Set.is_empty (Type_subst.free_var_finder env.cx t) then
        ...
        | TypeAppT { reason; use_op; type_; targs; from_value = _; use_desc = _ } ->
          Context.add_post_inference_validation_flow
            env.cx
            type_
            (SpecializeT (use_op, reason, reason, Some targs, Tvar.mk env.cx reason))
                  ...
                  if not (Context.ts_syntax env.cx) then
                    Flow_js_utils.add_output
                      ...
                      (Error_message.ETSSyntax
                         { kind = Error_message.TSClassAccessibility kind; loc = acc_loc }
                      );
