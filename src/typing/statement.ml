...
and statement cx = Ast.Statement.(
  ...
  let interface cx loc decl =
    ...
    let iface_sig = mk_interface_sig cx reason decl in
  ...
  let declare_class cx loc decl =
    ...
    let reason = DescFormat.instance_reason name name_loc in
    let class_sig = mk_declare_class_sig cx reason decl in
...
and mk_interface_sig cx reason decl =
  let open Class_sig in
...
and mk_declare_class_sig =
  ...
  let mk_mixins cx tparams_map (r, id, targs) =
    ...
    Anno.mk_super cx tparams_map props_bag targs
  in
    ...
    let iface_sig =
      ...
      let extends =
        ...
        | Some (_, {Ast.Type.Generic.id; targs}) ->
          ...
          Some (Anno.mk_super cx tparams_map i targs)
        | None ->
          None
      in
      let mixins =
