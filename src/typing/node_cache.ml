...
type cache = {
  ...
  match_pattern_unions: (Match_pattern_ir.PatternUnion.t * int) ALocMap.t;
}
...
let set_record_sig cache loc record =
  cache := { !cache with record_sigs = ALocMap.add loc record !cache.record_sigs }
...
let set_match_pattern_union cache loc v =
  cache := { !cache with match_pattern_unions = ALocMap.add loc v !cache.match_pattern_unions }
...
let get_record_sig cache loc = ALocMap.find_opt loc !cache.record_sigs
...
let get_match_pattern_union cache loc = ALocMap.find_opt loc !cache.match_pattern_unions
