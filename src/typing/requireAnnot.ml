...
let add_missing_annotation_error cx ~on_missing ?(hint_available = false) reason =
  on_missing ();
  Flow_js.add_output cx (Error_message.EMissingLocalAnnotation { reason; hint_available })
