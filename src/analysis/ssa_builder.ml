...
struct
  ...
  class ssa_builder ~enable_enums =
    object (this)
      ...
      method for_in_or_of_left_declaration left =
        ...
        | [(_, { Ast.Statement.VariableDeclaration.Declarator.id; init = _ })] ->
          let open Ast.Pattern in
          (match id with
          | (_, (Identifier _ | Object _ | Array _)) ->
            ignore @@ this#variable_declarator_pattern ~kind id
  ...
  let program_with_scope_and_jsx_pragma ~enable_enums ~jsx_ast program =
    let (loc, _) = program in
    let ssa_walk = new ssa_builder ~enable_enums in
