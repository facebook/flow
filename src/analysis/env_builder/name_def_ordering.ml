  ...
  *)
  ...
  class use_visitor ({ Env_api.env_values; env_entries; _ } as env) init =
    object (this)
      ...
      method! class_
          _
          ({ Ast.Class.id = ident; tparams; extends; implements; body; class_decorators; _ } as cls)
          =
        let open Flow_ast_mapper in
        let new_this =
          match ident with
  ...
  let depends ({ Env_api.providers; _ } as env) =
    ...
    let depends_of_fun fully_annotated function_ =
      depends_of_node (fun visitor -> visitor#function_def ~fully_annotated function_) ALocMap.empty
    in
    let depends_of_class
        fully_annotated
        { Ast.Class.id = ident; body; tparams; extends; implements; class_decorators; comments = _ }
        =
      depends_of_node
        (fun visitor ->
          let open Flow_ast_mapper in
