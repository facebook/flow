  ...
  *)
  ...
  class name_resolver
    ...
    object (this)
      ...
      method merge_completion_state_values first second =
        ...
        | (Some (AbruptCompletion.Break opt_label1), Some (AbruptCompletion.Continue opt_label2))
        | (Some (AbruptCompletion.Continue opt_label1), Some (AbruptCompletion.Break opt_label2))
        ...
        | Some abrupt_completion ->
          let merged_completion_state =
            Base.List.fold
              ...
              ~f:this#merge_completion_state_values
          in
          ...
          this#with_bindings ~lexical:true match_loc bindings (fun () ->
              ...
              (match
                 this#get_val_of_expression
                   ( match_keyword_loc,
                     Ast.Expression.Identifier (match_root_ident match_keyword_loc)
                   )
               with
              | Some refined_value ->
                env_state <-
      ...
      method private visit_match_pattern ~arg root_pattern =
        ...
        let impossible_refinement ~acc loc =
          ...
          | _ -> ()
        in
        let add_binding kind id bindings =
          let (_, { Ast.Identifier.name; _ }) = id in
          SMap.add ~combine:Base.List.rev_append name [(kind, id)] bindings
        in
        let bindings_of_rest bindings = function
          | Some (_, { RestPattern.argument = Some (_, { BindingPattern.kind; id; _ }); _ }) ->
            add_binding kind id bindings
          | _ -> bindings
        in
        let check_invalid_reference bindings id =
          let (loc, { Ast.Identifier.name; _ }) = id in
        ...
        let rec recurse acc pattern bindings non_binding_leaves =
          ...
          | (loc, IdentifierPattern id) ->
            ...
            let id_expr = (loc, Ast.Expression.Identifier id) in
          ...
          | (loc, MemberPattern x) ->
            let (mem_expr, root_ident) =
              Flow_ast_utils.expression_of_match_member_pattern
                ...
                x
            in
            check_invalid_reference bindings root_ident;
          ...
          | (_, AsPattern { AsPattern.pattern; target; _ }) ->
            let bindings =
              match target with
              | AsPattern.Binding (_, { BindingPattern.kind; id; _ }) ->
                add_binding kind id bindings
          ...
          let (static_body, instance_body) =
            Base.List.partition_tf body ~f:(function
                ...
                | Ast.Class.Body.DeclareMethod (_, { Ast.Class.DeclareMethod.static; _ }) ->
                  static
                | Ast.Class.Body.IndexSignature (_, { Ast.Type.Object.Indexer.static; _ }) -> static
      ...
      *)
        ...
        | (Some ref, Some refinement_key) ->
          if undef then (
            ...
            @@ this#optional_chain_must_be_able_to_refine_base_object_to_non_maybe
                 ~can_refine_obj_prop:OptionalChainingRefinement.CanApplyPropNonNullishRefi
                 arg;
            this#add_single_refinement refinement_key ~refining_locs:(L.LSet.singleton loc) ref;
      ...
      method optional_chain_must_be_able_to_refine_base_object_to_non_maybe
          ~can_refine_obj_prop (loc, expr) =
        let open Ast.Expression in
        this#record_member_read (loc, expr);
        let () =
          match expr with
          | OptionalMember _ ->
            this#member_expression_refinement_must_be_able_to_refine_base_object_to_non_maybe
              ~can_refine_obj_prop
              loc
              expr
              LookupMap.empty
      ...
      method member_expression_refinement_must_be_able_to_refine_base_object_to_non_maybe
          ~can_refine_obj_prop loc expr refis =
        let open Flow_ast.Expression in
