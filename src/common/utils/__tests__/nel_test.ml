...
let tests =
  ...
  >::: [
         ...
         ( "map"
         >:: (fun ctxt -> assert_equal ~ctxt (Nel.map (( * ) 2) lst |> Nel.to_list) [12; 8; 4]) );
         ( "ident_map"
         >:: fun ctxt ->
         ...
         ( "rev_map"
         >:: (fun ctxt -> assert_equal ~ctxt (Nel.rev_map (( + ) 1) lst |> Nel.to_list) [3; 5; 7])
         );
         ( "rev_append"
         >:: fun ctxt ->
         assert_equal ~ctxt (Nel.rev_append lst lst2 |> Nel.to_list) [2; 4; 6; 3; 5] );
         ( "append"
         >:: (fun ctxt -> assert_equal ~ctxt (Nel.append lst lst2 |> Nel.to_list) [6; 4; 2; 3; 5])
         );
