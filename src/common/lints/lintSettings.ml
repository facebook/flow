...
let set_value key value settings =
  ...
  in {settings with explicit_values = new_map}
...
let is_explicit lint_kind settings =
  LintMap.mem lint_kind settings.explicit_values

(* Iterate over all lint kinds with an explicit value *)
let iter f settings =
  LintMap.iter f settings.explicit_values

(* Fold over all lint kinds with an explicit value *)
let fold f settings acc =
  LintMap.fold f settings.explicit_values acc
...
let to_string settings =
  ...
  iter (fun kind (severity, _) ->
      Buffer.add_string acc (Printf.sprintf ", %s=%s"
        (string_of_kind kind) (string_of_severity severity)))
    settings;
  Buffer.contents acc
