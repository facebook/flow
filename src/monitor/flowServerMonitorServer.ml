...
module KeepAliveLoop = LwtLoop.Make (struct
  ...
  let rec wait_for_server_to_die monitor_options server =
    let pid = ServerInstance.pid_of server in
    let%lwt (_, status) = LwtSysUtils.blocking_waitpid pid in
    Memlog.suspend ();
    let%lwt () = wait_for_server_to_die_2 monitor_options server pid status in
    Memlog.resume ();
    Lwt.return_unit

  and wait_for_server_to_die_2 monitor_options server pid status =
    ...
    | Unix.WEXITED exit_status ->
      ...
      | Some exit_type ->
        ...
        let send_close conn =
          ...
          with e ->
            Memlog.log ("FlowServerMonitorServer.wait_for_server_to_die.2f EXCEPTION P.C.EXIT "
