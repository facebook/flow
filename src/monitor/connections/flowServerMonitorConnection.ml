...
module Make (ConnectionProcessor: CONNECTION_PROCESSOR) : CONNECTION
  ...
  let close_stream conn =
    Memlog.log ("Connection.close_stream.1 " ^ conn.name);
    try
  ...
  let write_and_close ~msg conn =
    Memlog.log ("Connection.write_and_close.1 " ^ conn.name);
  ...
  let stop_everything conn =
    Memlog.log ("Connection.stop_everything.1 " ^ conn.name);
    close_stream conn;
  ...
  let close_immediately conn =
    Memlog.log ("Connection.close_immediately.1 " ^ conn.name);
  ...
  | WriteAndClose msg ->
    Memlog.log ("Connection.handle_command.WriteAndClose.1 " ^ conn.name);
    Lwt.cancel conn.command_thread;
  ...
  let flush_and_close conn =
    Memlog.log ("Connection.flush_and_close.1 " ^ conn.name);
    stop_everything conn;
  ...
  let wait_for_closed conn =
    ...
    Memlog.log ("Connection.wait_for_closed.2 " ^ conn.name);
    Lwt.return_unit
    ...
    let catch conn exn =
      ...
      | Lwt_stream.Empty as exn ->
        Memlog.log ("Connection.CommandLoop.catch.1 "
          ^ conn.name ^ " " ^ (Printexc.to_string exn));
        Lwt.return_unit
      | exn -> begin
  ...
  module ReadLoop = LwtLoop.Make (struct
    ...
    let catch connection exn =
      Memlog.log ("Connection.ReadLoop.catch.1 "
        ^ connection.name ^ " " ^ (Printexc.to_string exn));
      Logger.error
        ~exn
  ...
  let create ~name ~in_fd ~out_fd ~close ~on_read =
    let wait_for_closed_thread, close =
      ...
      let close () =
        Memlog.log ("Connection.close.1 " ^ name);
        let%lwt () = close () in
