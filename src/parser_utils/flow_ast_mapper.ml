  ...
  object (this)
    ...
    method statement (stmt : ('loc, 'loc) Ast.Statement.t) =
      Ast.Statement.(
        ...
        | (loc, FunctionDeclaration func) ->
          id_loc this#function_declaration loc func stmt (fun func ->
              (loc, FunctionDeclaration func))
        | (loc, If if_stmt) ->
        ...
        | (loc, VariableDeclaration decl) ->
          id_loc this#variable_declaration loc decl stmt (fun decl ->
              (loc, VariableDeclaration decl))
    ...
    method expression (expr : ('loc, 'loc) Ast.Expression.t) =
      Ast.Expression.(
        ...
        | (loc, JSXFragment x) ->
          id_loc this#jsx_fragment loc x expr (fun x -> (loc, JSXFragment x))
        ...
        | (loc, OptionalCall x) ->
          id (this#optional_call loc) x expr (fun x -> (loc, OptionalCall x))
    ...
    method enum_boolean_body (body : 'loc Ast.Statement.EnumDeclaration.BooleanBody.t) =
      Ast.Statement.EnumDeclaration.BooleanBody.(
    ...
    method enum_number_body (body : 'loc Ast.Statement.EnumDeclaration.NumberBody.t) =
      Ast.Statement.EnumDeclaration.NumberBody.(
        let { members; explicitType = _ } = body in
    ...
    method enum_string_body (body : 'loc Ast.Statement.EnumDeclaration.StringBody.t) =
      Ast.Statement.EnumDeclaration.StringBody.(
        let { members; explicitType = _ } = body in
        let members' =
          match members with
    ...
    method enum_symbol_body (body : 'loc Ast.Statement.EnumDeclaration.SymbolBody.t) =
      Ast.Statement.EnumDeclaration.SymbolBody.(
        let { members } = body in
    ...
    method enum_defaulted_member (member : 'loc Ast.Statement.EnumDeclaration.DefaultedMember.t) =
      Ast.Statement.EnumDeclaration.DefaultedMember.(
    ...
    method enum_boolean_member
      ...
      Ast.Statement.EnumDeclaration.InitializedMember.(
      ...
      Ast.Statement.ImportDeclaration.(
        ...
        | ImportNamedSpecifiers named_specifiers ->
          let named_specifiers' =
            ListUtils.ident_map this#import_named_specifier named_specifiers
          in
    ...
    method jsx_opening_attribute (jsx_attr : ('loc, 'loc) Ast.JSX.Opening.attribute) =
      Ast.JSX.Opening.(
        ...
        | SpreadAttribute (loc, attr) ->
          id_loc this#jsx_spread_attribute loc attr jsx_attr (fun attr ->
              SpreadAttribute (loc, attr)))
      ...
      Ast.Pattern.Object.Property.(
        ...
        | Identifier identifier ->
          id (this#pattern_object_property_identifier_key ?kind) identifier key (fun id' ->
              Identifier id')
        | Computed expr ->
          id (this#pattern_object_property_computed_key ?kind) expr key (fun expr' ->
              Computed expr'))
    ...
    method pattern_object_rest_property
      ...
      Ast.Pattern.Object.RestProperty.(
