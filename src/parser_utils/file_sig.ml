...
= struct
  ...
  and require =
    ...
    | ImportDynamic of { source: L.t Ast_utils.source; import_loc: L.t }
  ...
  and named_export_kind =
    | NamedDeclaration
    | NamedSpecifier of { local: L.t Ast_utils.ident; source: L.t Ast_utils.source option }

  and export_star =
    | ExportStar of { star_loc: L.t; source: L.t Ast_utils.source }
  ...
  let to_string t =
    let string_of_module_sig module_sig =
      ...
      in
      let string_of_export (n, export) = n, match export with
      ...
      in
      let string_of_type_export (n, type_export) = n, match type_export with
  ...
  let require_loc_map msig =
    ...
    | ES { named; star } ->
      (* export {...} from 'foo' *)
      let acc = List.fold_left (fun acc (_, export) ->
        match export with
  ...
  *)
  class requires_exports_calculator ~ast ~module_ref_prefix = object(this)
    inherit [(exports_info t', error) result, L.t] visitor ~init:(Ok (mk_file_sig init_exports_info)) as super
    ...
    method! declare_export_declaration stmt_loc (decl: (L.t, L.t) Ast.Statement.DeclareExportDeclaration.t) =
    ...
    method handle_assignment ~(is_toplevel: bool) loc (expr: (L.t, L.t) Ast.Expression.Assignment.t) =
      let open Ast.Expression in
  ...
  class mapper = object(this)
    ...
    method export (export: string * (L.t * export)) =
      ...
      | n, (export_loc, ExportDefault { default_loc; local }) ->
        ...
        if export_loc == export_loc' && default_loc == default_loc' && local == local'
        then export
        else n, (export_loc', ExportDefault { default_loc = default_loc'; local = local' })
      | n, (export_loc, ExportNamed { loc; kind }) ->
        let export_loc' = this#loc export_loc in
        let loc' = this#loc loc in
        let kind' = this#named_export_kind kind in
        if export_loc == export_loc' && loc == loc' && kind == kind'
        then export
        else n, (export_loc', ExportNamed { loc = loc'; kind = kind' })
      | n, (export_loc, ExportNs { loc; star_loc; source; }) ->
        ...
        then export
        else n, (export_loc', ExportNs { loc = loc'; star_loc = star_loc'; source = source'; })
    ...
    method export_star (export_star: L.t * export_star) =
      ...
      | export_loc, ExportStar { star_loc; source } ->
        ...
        let star_loc' = this#loc star_loc in
        let source' = this#source source in
        if export_loc == export_loc' && star_loc == star_loc' && source == source'
        then export_star
        else export_loc', ExportStar { star_loc = star_loc'; source = source'; }
      ...
      | SignatureVerificationError sve ->
        ...
        begin match sve with
          ...
          | UnexpectedArrayHole loc ->
            ...
            if loc == loc'
            then tolerable_error
            else SignatureVerificationError (UnexpectedArrayHole loc')
          | EmptyArray loc ->
            let loc' = this#loc loc in
            if loc == loc'
            then tolerable_error
          ...
          | EmptyObject loc ->
            ...
            if loc == loc'
            then tolerable_error
            else SignatureVerificationError (EmptyObject loc')
...
let abstractify_locs: With_Loc.t -> With_ALoc.t =
  ...
  | WL.Import0 { source } ->
    WA.Import0 { source = abstractify_fst source }
  | WL.Import {
    import_loc;
    source;
    named;
  ...
  | WL.NamedSpecifier { local; source } ->
    WA.NamedSpecifier {
      local = abstractify_fst local;
      source = Option.map ~f:abstractify_fst source;
    }
  in
  let abstractify_export = function
  | WL.ExportDefault { default_loc; local } ->
    WA.ExportDefault {
      default_loc = ALoc.of_loc default_loc;
      local = Option.map ~f:abstractify_fst local;
    }
  | WL.ExportNamed { loc; kind } ->
    WA.ExportNamed {
      loc = ALoc.of_loc loc;
      kind = abstractify_named_export_kind kind;
    }
  ...
  | WL.ES { named; star } ->
    WA.ES {
      named = abstractify_named_exports named;
      star = abstractify_es_star star;
    }
  in
  let abstractify_type_export = function WL.TypeExportNamed { loc; kind } ->
