...
class virtual ['L] mapper = object(this)
  inherit ['L] Flow_ast_mapper.mapper as super
  method virtual loc_annot_contains_target : 'L -> bool

  method! program ((l, _, _) as x) =
    if this#loc_annot_contains_target l then super#program x else x

  method! statement ((l,_) as x) =
    if this#loc_annot_contains_target l then super#statement x else x

  method! comment ((l, _) as x)=
    if this#loc_annot_contains_target l then super#comment x else x

  method! expression ((l, _) as x)=
    if this#loc_annot_contains_target l then super#expression x else x
end
