...
let tests =
  [
    ...
    ( "simple_with_attribute_and_child"
    >:: (fun ctxt -> assert_expression_string ~ctxt ~pretty:true "<A a=\"a\"><B /></A>") );
    ( "long_attribute_with_children"
    >:: fun ctxt ->
    ...
    ( "borderline_length_with_children"
    >:: fun ctxt ->
    ...
    *)
    ...
    >:: fun ctxt ->
    assert_expression_string ~ctxt ~pretty:true "<A a=\"a\">\n  a{\" \"}\n  b\n</A>" );
    ( "children_fit_on_one_line"
    >:: (fun ctxt -> assert_expression_string ~ctxt ~pretty:true "<A><B /><C /></A>") );
    ( "long_child_element"
    >:: fun ctxt ->
    ...
    ( "attribute_spread_between_attributes"
    >:: (fun ctxt -> assert_expression_string ~ctxt "<A a {...a}b />") );
    ("multiple_shorthand_attributes" >:: (fun ctxt -> assert_expression_string ~ctxt "<A a b />"));
    ( "shorthand_and_longhand_attributes"
    >:: (fun ctxt -> assert_expression_string ~ctxt "<A a b=\"\"/>") );
    ( "multiple_longhand_attributes"
    >:: fun ctxt ->
    ...
    assert_expression_string
      ...
      ~pretty:true
      ("<A\n  a=\"" ^ String.make 80 'a' ^ "\"\n  b=\"b\"\n/>") );
    ( "string_longhand_and_shorthand_attributes"
    >:: fun ctxt ->
