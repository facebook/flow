...
let tests =
  ...
  >::: [ "operator_precedence" >::: Operator_precedence_test.tests;
         ...
         ( "do_while_single_statement_long"
         >:: fun ctxt ->
         (* do xxxx...; while (yyyy...) *)
         let x80 = String.make 80 'x' in
         let y80 = String.make 80 'y' in
         ...
         let x = E.identifier "x" in
         (* `(x++).y` *)
         let update = E.member_expression (E.member (E.increment ~prefix:false x) ~property:"y") in
         ...
         ( "if_statement_with_empty_consequent"
         >:: fun ctxt ->
         let layout = Js_layout_generator.statement (S.if_ (E.identifier "x") (S.empty ()) None) in
         ...
         assert_statement
           ...
           ("{\n" ^ "  if (\n" ^ "    " ^ a80 ^ "\n" ^ "  )\n" ^ "    y;\n" ^ "  z;\n" ^ "}")
           ast );
         ( "if_else_statement_with_empty_consequent"
         >:: fun ctxt ->
         let layout =
           Js_layout_generator.statement
         ...
         ( "if_else_statement_with_empty_alternate"
         >:: fun ctxt ->
         let layout =
           Js_layout_generator.statement
         ...
         ( "if_else_statement_with_empty_consequent_and_alternate"
         >:: fun ctxt ->
         let layout =
           Js_layout_generator.statement
         ...
         assert_output
           ...
           ("for (var " ^ a80 ^ " in b) {\n" ^ "  " ^ a80 ^ ";\n" ^ "}")
           layout );
         ( "forin_statement_pattern_identifier"
         >:: fun ctxt ->
         let mk_layout a b =
           Js_layout_generator.statement
         ...
         let layout =
           Js_layout_generator.statement
             (S.for_in (S.for_in_pattern (Patterns.identifier "a")) (E.identifier "b") (S.empty ()))
         in
         assert_output ~ctxt "for(a in b);" layout;
         assert_output ~ctxt ~pretty:true "for (a in b);" layout );
         ( "forof_statement_declaration"
         >:: fun ctxt ->
         let mk_layout a b =
           Js_layout_generator.statement
             (S.for_of
         ...
         assert_output
           ...
           ("for (var " ^ a80 ^ " of b) {\n" ^ "  " ^ a80 ^ ";\n" ^ "}")
           layout );
         ( "forof_statement_pattern_identifier"
         >:: fun ctxt ->
         let mk_layout a b =
           Js_layout_generator.statement
         ...
         let layout =
           Js_layout_generator.statement
             (S.for_of (S.for_of_pattern (Patterns.identifier "a")) (E.identifier "b") (S.empty ()))
         in
         assert_output ~ctxt "for(a of b);" layout;
         assert_output ~ctxt ~pretty:true "for (a of b);" layout );
         ( "yield_expressions"
      ...
      assert_statement_string ~ctxt "export*as foo,{bar}from'a';"; *);
         ...
         >:: fun ctxt ->
         let layout = Js_layout_generator.expression (E.typecast (E.identifier "a") Types.mixed) in
         assert_layout
           ~ctxt
           L.(
             loc
         ...
         let a80 = String.make 80 'a' in
         let layout = Js_layout_generator.expression (E.typecast (E.identifier a80) Types.mixed) in
         ...
         >:: (fun ctxt -> assert_statement_string ~ctxt "declare module.exports:a;") );
         ...
         ( "with_statement_with_empty_body"
         >:: fun ctxt ->
