  ...
  | (_, Array { Array.elements; annot = _; comments = _ }) ->
    Array.(
      List.fold_left
        (fun acc -> function
          | Hole _ -> acc
          | Element (_, { Element.argument = p; default = _ }) -> acc @ pattern loc p
          | RestElement (_, { RestElement.argument = p; comments = _ }) -> acc @ pattern loc p)
        []
        elements)
  | (_, Expression _) -> []
...
let function_declaration
    loc { Ast.Function.id; generator; async; tparams; params; return; body; predicate; sig_loc; _ }
    =
  ( Base.Option.value_exn id,
    (loc, Kind.FunctionDef { generator; async; tparams; params; return; body; predicate; sig_loc })
  )
...
let class_ loc class_ =
  ...
  let { id; tparams; body; extends; implements; class_decorators = _; comments = _ } = class_ in
