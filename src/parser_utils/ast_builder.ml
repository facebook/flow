...
module Types = struct
  module Functions = struct
    ...
    let make ?tparams ?comments ?(effect_ = Ast.Function.Arbitrary) params return =
      { Ast.Type.Function.tparams; params; return; effect_; comments }
  end
...
module Statements = struct
  ...
  let import_declaration
      ?(loc = Loc.none) ?comments ?attributes import_kind source default specifiers =
    ( loc,
      ImportDeclaration
  ...
  let component_declaration
      ?(loc = Loc.none) ?(async = false) ?tparams ?params ?renders ?comments id body =
  ...
  let synthesized_component_declaration
      ~use_component_syntax ?tparams ~typed_props ~body new_component_name =
    let component_body = (Loc.none, { Flow_ast.Statement.Block.body; comments = None }) in
    if use_component_syntax then
      ...
      let params =
        if Base.List.is_empty typed_props then
          None
        else
          let properties =
            Base.List.map typed_props ~f:(fun (def, _) ->
                Object.Property
                  ( Loc.none,
                    {
