...
let program
  ...
  *)
  ...
  let diff_and_recurse
    ...
    Option.(list_diff algo old_list new_list >>= recurse_into_diff f trivial old_list 0)
  in
  ...
  let diff_and_recurse_nonopt (type a b) (f : a -> a -> b change list) =
    diff_and_recurse (fun x y -> f x y |> Option.return)
  in
  ...
  and export_named_declaration export1 export2 =
    Ast.Statement.ExportNamedDeclaration.(
      let { declaration = decl1; specifiers = specs1; source = src1; exportKind = kind1 } =
        export1
      in
      let { declaration = decl2; specifiers = specs2; source = src2; exportKind = kind2 } =
        export2
      in
  ...
  and declare_export
    ...
    Ast.Statement.DeclareExportDeclaration.(
      let { default = default1; declaration = decl1; specifiers = specs1; source = src1 } =
        export1
      in
      let { default = default2; declaration = decl2; specifiers = specs2; source = src2 } =
        export2
      in
  ...
  and import_declaration
    ...
    Ast.Statement.ImportDeclaration.(
      let { importKind = imprt_knd1; source = src1; default = dflt1; specifiers = spec1 } =
        import1
      in
      let { importKind = imprt_knd2; source = src2; default = dflt2; specifiers = spec2 } =
        import2
      in
  ...
  and class_element
    ...
    Ast.Class.Body.(
      ...
      | (Property p1, Property p2) -> class_property p1 p2 |> Option.return
      | _ -> None)
  (* TODO *)
  ...
  and object_key key1 key2 =
    ...
    | (EOP.Identifier i1, EOP.Identifier i2) -> diff_if_changed identifier i1 i2 |> Option.return
    | (EOP.Computed e1, EOP.Computed e2) -> diff_if_changed expression e1 e2 |> Option.return
    | (_, _) -> None
  ...
  and identifier (id1 : (Loc.t, Loc.t) Ast.Identifier.t) (id2 : (Loc.t, Loc.t) Ast.Identifier.t) :
    ...
    in
    let comments = syntax_opt old_loc comments1 comments2 |> Option.value ~default:[] in
    comments @ name
  and conditional
  ...
  and pattern_object_property_key
    ...
    | (POP.Literal (loc, l1), POP.Literal (_, l2)) ->
      diff_if_changed (literal loc) l1 l2 |> Option.return
    | (POP.Identifier i1, POP.Identifier i2) -> diff_if_changed identifier i1 i2 |> Option.return
    | (POP.Computed e1, POP.Computed e2) -> diff_if_changed expression e1 e2 |> Option.return
    | (_, _) -> None
  and pattern_array
  ...
  and type_
    ...
    Ast.Type.(
      let type_diff =
        ...
        | _ -> None
      in
      Option.value type_diff ~default:[(loc1, Replace (Type (loc1, type1), Type (loc1, type2)))])
  and interface_type
  ...
  and generic_identifier_type
    ...
    Ast.Type.Generic.Identifier.(
      ...
      | (Unqualified id1, Unqualified id2) -> diff_if_changed identifier id1 id2 |> Option.return
  ...
  and opaque_type
    ...
    Ast.Statement.OpaqueType.(
      let { id = id1; tparams = t_params1; impltype = impltype1; supertype = supertype1 } =
        o_type1
      in
      let { id = id2; tparams = t_params2; impltype = impltype2; supertype = supertype2 } =
        o_type2
      in
      let id_diff = diff_if_changed identifier id1 id2 |> Option.return in
      let t_params_diff = diff_if_changed_opt type_parameter_declaration t_params1 t_params2 in
