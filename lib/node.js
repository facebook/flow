 ...
 * Portions copyright Node.js contributors. All rights reserved.
 *
...
declare class http$IncomingMessage<SocketT = net$Socket> extends stream$Readable {
  ...
  complete: boolean;
  method: string;
  trailers: { [key: string]: string | void };
  setTimeout(msecs: number, callback: () => mixed): void;
  connection: SocketT;
  socket: SocketT;
  statusCode: number;
...
declare class http$ServerResponse extends stream$Writable {
  addTrailers(headers: http$OutgoingHttpHeaders | Array<[string, string]>): void;
  ...
  statusMessage: string;
  writeContinue(): void;
  writeHead(status: number, statusMessage?: string, headers?: http$OutgoingHttpHeaders): void;
...
type requestOptions = {|
  ...
  socketPath?: string,
  timeout?: number,
  agent?: http$Agent<mixed> | boolean,
  createConnection?: (options: requestOptions, oncreate: (err: Error, socket: net$Socket) => mixed) => net$Socket;
|}
...
type http2$IncomingHttpStatusHeader = {
  ":status"?: number,
}

type http2$IncomingHttpHeaders = http$IncomingHttpHeaders & {
  ":path"?: string,
  ":method"?: string,
  ":authority"?: string,
  ":scheme"?: string,
...
type http2$StreamPriorityOptions = {
  exclusive?: boolean,
  parent?: number,
  weight?: number,
  silent?: boolean,
...
type http2$ClientSessionRequestOptions = {
  endStream?: boolean;
  exclusive?: boolean;
  parent?: number;
  weight?: number;
...
type http2$StatOptions = {
  offset: number,
  length: number,
  ...
}

type http2$ServerStreamResponseOptions = {
  endStream?: boolean,
  waitForTrailers?: boolean,
  ...
}

type http2$ServerStreamFileResponseOptions = {
  statCheck?: (
    stats: fs$Stats,
    headers: http$OutgoingHttpHeaders,
...
type http2$ServerStreamFileResponseOptionsWithError = http2$ServerStreamFileResponseOptions & {
  onError?: (err: ErrnoError) => mixed;
  ...
}

type http2$SessionOptions = {|
  ...
  createConnection?: (authority: URL, option: http2$SessionOptions) => stream$Duplex,
|};

type http2$ClientSessionOptions = {|
  ...http2$SessionOptions,
  maxReservedRemoteStreams?: number,
  createConnection?: (authority: URL, option: http2$SessionOptions) => stream$Duplex,
|}

type http2$ServerSessionOptions = {|
  ...http2$SessionOptions,
  Http1IncomingMessage?: typeof http$IncomingMessage;
  Http1ServerResponse?: typeof http$ServerResponse;
  Http2ServerRequest?: typeof http2$ServerRequest;
  Http2ServerResponse?: typeof http2$ServerResponse;
|}

type http2$SecureClientSessionOptions = {...http2$ClientSessionOptions, ...} /*TODO: & tls$ConnectionOptions */
type http2$SecureServerSessionOptions = {...http2$ServerSessionOptions, ...} /*TODO: & tls$TlsOptions */

type http2$createServerOptions = {
  ...http2$ServerSessionOptions,
  ...
}

type http2$SecureServerOptions = http2$SecureServerSessionOptions & {
  allowHTTP1?: boolean,
  origins?: string[],
  ...
}

declare class http2$Server extends net$Server {
  setTimeout(msec?: number, callback?: () => mixed): this;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "checkContinue", listener: (request: http2$ServerRequest, response: http2$ServerResponse) => mixed): this;
  // on(event: "request", listener: (request: http2$ServerRequest, response: http2$ServerResponse) => mixed): this;
  // on(event: "session", listener: (session: http2$ServerSession) => mixed): this;
  // on(event: "sessionError", listener: (err: Error) => mixed): this;
  // on(event: "stream", listener: (stream: http2$ServerStream, headers: http2$IncomingHttpHeaders, flags: number) => mixed): this;
  // on(event: "timeout", listener: () => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

declare class http2$SecureServer extends tls$Server {
  setTimeout(msec?: number, callback?: () => mixed): this;
...
declare class http2$Session extends events$EventEmitter {
  +type: number;
  +alpnProtocol?: string;
  +closed: boolean;
  +connecting: boolean;
  +destroyed: boolean;
  +encrypted?: boolean;
  +localSettings: http2$Settings;
  +originSet?: string[];
  +pendingSettingsAck: boolean;
  +remoteSettings: http2$Settings;
  +socket: net$Socket | tls$TLSSocket;
  +state: http2$SessionState;

  close(callback?: () => mixed): void;
  destroy(error?: Error, code?: number): void;
  goaway(code?: number, lastStreamID?: number, opaqueData?: Buffer | DataView | $TypedArray): void;
  ping(callback: (err: Error | null, duration: number, payload: Buffer) => mixed): boolean;
  ping(payload: Buffer | DataView | $TypedArray , callback: (err: Error | null, duration: number, payload: Buffer) => mixed): boolean;
  ref(): void;
  setTimeout(msecs: number, callback?: () => mixed): void;
  settings(settings: http2$Settings): void;
  unref(): void;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "close", listener: () => mixed): this;
  // on(event: "error", listener: (err: Error) => mixed): this;
  // on(event: "frameError", listener: (frameType: number, errorCode: number, streamID: number) => mixed): this;
  // on(event: "goaway", listener: (errorCode: number, lastStreamID: number, opaqueData: Buffer) => mixed): this;
  // on(event: "localSettings", listener: (settings: http2$Settings) => mixed): this;
  // on(event: "ping", listener: () => mixed): this;
  // on(event: "remoteSettings", listener: (settings: http2$Settings) => mixed): this;
  // on(event: "timeout", listener: () => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

type http2$AlternativeServiceOptions = {
  origin: number | string | URL,
  ...
}

declare class http2$ServerSession extends http2$Session {
  altsvc(alt: string, originOrStream: number | string | URL | http2$AlternativeServiceOptions): void;
  origin(...args: Array<string | URL | { origin: string, ... }>): void;
  +server: http2$Server | http2$SecureServer;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "connect", listener: (session: http2$ServerSession, socket: net$Socket | tls$TLSSocket) => mixed): this;
  // on(event: "stream", listener: (stream: http2$ServerStream, headers: http2$IncomingHttpHeaders, flags: number) => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

declare class http2$ClientSession extends http2$Session {
  request(headers?: http$OutgoingHttpHeaders, options?: http2$ClientSessionRequestOptions): http2$ClientStream;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "altsvc", listener: (alt: string, origin: string, stream: number) => mixed): this;
  // on(event: "origin", listener: (origins: string[]) => mixed): this;
  // on(event: "connect", listener: (session: http2$ClientSession, socket: net$Socket | tls$TLSSocket) => mixed): this;
  // on(event: "stream", listener: (stream: http2$ClientStream, headers: http2$IncomingHttpHeaders & http2$IncomingHttpStatusHeader, flags: number) => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

declare class http2$Stream extends stream$Duplex {
  +aborted: boolean;
  +bufferSize: number;
  +closed: boolean;
  +destroyed: boolean;
  ...
  +pending: boolean;
  +rstCode: number;
  +sentHeaders: http$OutgoingHttpHeaders;
  +sentInfoHeaders?: http$OutgoingHttpHeaders[];
  +sentTrailers?: http$OutgoingHttpHeaders;
  +session: http2$Session;
  +state: http2$StreamState;

  close(code?: number, callback?: () => mixed): void;
  priority(options: http2$StreamPriorityOptions): void;
  setTimeout(msecs: number, callback?: () => mixed): void;
  sendTrailers(headers: http$OutgoingHttpHeaders): void;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "aborted", listener: () => mixed): this;
  // on(event: "close", listener: () => mixed): this;
  // on(event: "data", listener: (chunk: Buffer | string) => mixed): this;
  // on(event: "drain", listener: () => mixed): this;
  // on(event: "end", listener: () => mixed): this;
  // on(event: "error", listener: (err: Error) => mixed): this;
  // on(event: "finish", listener: () => mixed): this;
  // on(event: "frameError", listener: (frameType: number, errorCode: number) => mixed): this;
  // on(event: "pipe", listener: (src: stream$Readable) => mixed): this;
  // on(event: "unpipe", listener: (src: stream$Readable) => mixed): this;
  // on(event: "streamClosed", listener: (code: number) => mixed): this;
  // on(event: "timeout", listener: () => mixed): this;
  // on(event: "trailers", listener: (trailers: http2$IncomingHttpHeaders, flags: number) => mixed): this;
  // on(event: "wantTrailers", listener: () => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

declare class http2$ClientStream extends http2$Stream {
  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "continue", listener: () => mixed): this;
  // on(event: "headers", listener: (headers: http2$IncomingHttpHeaders & http2$IncomingHttpStatusHeader, flags: number) => mixed): this;
  // on(event: "push", listener: (headers: http2$IncomingHttpHeaders, flags: number) => mixed): this;
  // on(event: "response", listener: (headers: http2$IncomingHttpHeaders & http2$IncomingHttpStatusHeader, flags: number) => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

declare class http2$ServerStream extends http2$Stream {
  +headersSent: boolean;
  +pushAllowed: boolean;
  additionalHeaders(headers: http$OutgoingHttpHeaders): void;
  pushStream(headers: http$OutgoingHttpHeaders, callback?: (err: Error | null, pushStream: http2$ServerStream, headers: http$OutgoingHttpHeaders) => mixed): void;
  pushStream(headers: http$OutgoingHttpHeaders, options?: http2$StreamPriorityOptions, callback?: (err: Error | null, pushStream: http2$ServerStream, headers: http$OutgoingHttpHeaders) => mixed): void;
  respond(headers?: http$OutgoingHttpHeaders, options?: http2$ServerStreamResponseOptions): void;
  respondWithFD(fd: number, headers?: http$OutgoingHttpHeaders, options?: http2$ServerStreamFileResponseOptions): void;
  respondWithFile(path: string, headers?: http$OutgoingHttpHeaders, options?: http2$ServerStreamFileResponseOptionsWithError): void;
}

declare class http2$ServerRequest extends stream$Readable {
  +aborted: boolean;
  +authority: string;
  ...
  setTimeout(msecs: number, callback?: () => mixed): void;
  +socket: net$Socket | tls$TLSSocket;
  +stream: http2$ServerStream;
  +trailers: http2$IncomingHttpHeaders;
  +url: string;
  read(size?: number): Buffer | string | null;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "aborted", listener: (hadError: boolean, code: number) => mixed): this;
  // on(event: "close", listener: () => mixed): this;
  // on(event: "data", listener: (chunk: Buffer | string) => mixed): this;
  // on(event: "end", listener: () => mixed): this;
  // on(event: "readable", listener: () => mixed): this;
  // on(event: "error", listener: (err: Error) => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
}

declare class http2$ServerResponse extends stream$Stream {
  ...
  end(callback?: () => mixed): void;
  end(data: string | Buffer | Uint8Array, callback?: () => mixed): void;
  end(data: string | Buffer | Uint8Array, encoding: string, callback?: () => mixed): void;
  getHeader(name: string): string;
  ...
  setTimeout(msecs: number, callback?: () => mixed): void;
  write(chunk: string | Buffer | Uint8Array, callback?: (err: Error) => mixed): boolean;
  write(chunk: string | Buffer | Uint8Array, encoding: string, callback?: (err: Error) => mixed): boolean;
  writeContinue(): void;
  ...
  createPushResponse(headers: http$OutgoingHttpHeaders, callback: (err: Error | null, res: http2$ServerResponse) => mixed): void;

  // TODO: add event emitter handlers when bug with overloading will be fixed
  // on(event: "close", listener: () => mixed): this;
  // on(event: "drain", listener: () => mixed): this;
  // on(event: "error", listener: (error: Error) => mixed): this;
  // on(event: "finish", listener: () => mixed): this;
  // on(event: "pipe", listener: (src: stream$Readable) => mixed): this;
  // on(event: "unpipe", listener: (src: stream$Readable) => mixed): this;
  // on(event: string, listener: (...args: any[]) => mixed): this;
...
declare module "http2" {
  declare var Http2ServerRequest: Class<http2$ServerRequest>;
  declare var Http2ServerResponse: Class<http2$ServerResponse>;

  declare function getDefaultSettings(): http2$Settings;
  declare function getPackedSettings(settings: http2$Settings): Buffer;
  declare function getUnpackedSettings(buf: Buffer | Uint8Array): http2$Settings;

  declare function createServer(
    onRequestHandler?: (
      request: http2$ServerRequest,
      response: http2$ServerResponse
    ) => mixed
  ): http2$Server;
  declare function createServer(
    options: http2$createServerOptions,
    onRequestHandler?: (
      request: http2$ServerRequest,
      response: http2$ServerResponse
    ) => mixed
  ...
  declare function createSecureServer(
    onRequestHandler?: (
      request: http2$ServerRequest,
      response: http2$ServerResponse
    ) => mixed
  ...
  declare function connect(
    ...
    listener?: (session: http2$ClientSession, socket: net$Socket | tls$TLSSocket) => mixed): http2$ClientSession;
  declare function connect(
    authority: string | URL,
    options?: http2$ClientSessionOptions | http2$SecureClientSessionOptions,
    listener?: (session: http2$ClientSession, socket: net$Socket | tls$TLSSocket) => mixed,
  ): http2$ClientSession;
