...
declare class React$Component<Props, State = void> {
  ...
  static getDerivedStateFromProps(props: Props, state: State): $FIXME$NonMaybeObject<$Shape<State>> | null;
  getSnapshotBeforeUpdate(prevProps: Props, prevState: State): $FIXME$NonMaybeMixed | null;
  componentDidCatch(
    error: Error,
    info: {
...
// This is a workaround for $NonMaybeType<T> not working when T is of the form $Shape<U>
// https://github.com/facebook/flow/issues/6606
type $FIXME$NonMaybeObject<T> = T & Object;

// $NonMaybeType<mixed> is currently equivalent to mixed, but should exclude void and null
// https://github.com/facebook/flow/issues/6606#issuecomment-405690050
type $FIXME$NonMaybeMixed = $NonMaybeType<mixed>;
