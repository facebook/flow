...
fn spec() -> flow_command_spec::Spec {
    ...
    let spec = flow_command_utils::add_options_flags(spec);
...
fn main(
    ...
    let options = Arc::new(flow_command_utils::make_options(
    ...
    let server_log_file = match server_log_file.or_else(|| std::env::var("FLOW_LOG_FILE").ok()) {
        Some(s) => s,
        None => flow_command_utils::server_log_file(
            &flowconfig_name,
    ...
    let monitor_log_file =
        match monitor_log_file.or_else(|| std::env::var("FLOW_MONITOR_LOG_FILE").ok()) {
            Some(s) => s,
            None => flow_command_utils::monitor_log_file(
                &flowconfig_name,
                options.temp_dir.as_str(),
                options.root.as_path(),
            ),
    ...
    let file_watcher_mergebase_with = Some(flow_command_utils::choose_file_watcher_mergebase_with(
        &flowconfig,
        vcs,
...
pub(crate) fn command() -> flow_command_spec::Command {
    flow_command_spec::command(spec(), |args| {
        flow_command_utils::maybe_run_in_cgroup(args);
        ...
        main(
            ...
            flow_command_utils::get_options_flags(args),
