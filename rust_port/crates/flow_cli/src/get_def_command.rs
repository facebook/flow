...
fn parse_args(
    ...
    let (file, line, column) = match args.as_slice() {
        ...
        [line, column] => (
            flow_command_utils::get_file_from_filename_or_stdin("get-def", None, path),
            line.parse::<i32>().unwrap(),
            column.parse::<i32>().unwrap(),
        ),
        _ => {
            let msg = flow_command_spec::command(spec(), |_| {}).string_of_usage();
    ...
    let (line, column) = flow_command_utils::convert_input_pos(line, column);
    (file, line, column)
}
