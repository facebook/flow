...
module Obj_rule = struct
  ...
  let string_of_cond (cond : cond_t) : string =
    (Utils.string_of_expr (Loc.none, (fst cond))) ^
    " : " ^
    (Utils.string_of_type (Loc.none, (snd cond)))
    |> Str.global_replace (Str.regexp "\n") ""

  (* We don't do fix-point calculation yet *)
  let merge_cond (facts : cond_t list) (new_cond : cond_t list) : cond_t list =
    facts @ new_cond
  ...
  let is_valid (facts : cond_t list) (cond : cond_t) : bool =
    List.mem cond facts

  (* Simply check whether the consequence of a rule has the given *)
  (* condition *)
  let cond_to_rule (all_rules : rule_t list) (cond : cond_t) : rule_t =
    List.filter (fun r -> List.mem cond r.cons) all_rules
    |> List.hd

  let exercise (rule : rule_t) : (code_t * cond_t list) =
    (rule.grammar, rule.cons)
...
let parse_type_rule cons  : Gen.cond_t =
  ...
  parse_helper Parser_flow.Parse.expression expr,
  parse_helper Type_parser._type etype
