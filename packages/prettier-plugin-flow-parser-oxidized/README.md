...
The `index.mjs` shipped here is the prebuilt, minified plugin bundle from the
upstream fork (see `facebook/INTERNAL_DEV.md`). It is `@generated` and is
preserved as-is from upstream during this fork — **do not hand-edit it**.
...
into the Flow Rust parser instead of the hermes WASM parser. Until that
retargeting lands, the bundle still references `hermes` as the prettier parser
name, which is why `__tests__/*.js` keep `parser: 'hermes'` in their options.
