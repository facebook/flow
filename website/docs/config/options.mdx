...
Set this to `true` to indicate that Flow should interpret object types as exact by default.
...
In versions 0.200 and 0.201, the flag is required to be explicitly set to either `true` or `false`.
From version 0.202, the default value will be `true`.
...
Changes the default type of `catch` variables from [`any`](../any) to [`mixed`](../mixed). E.g.
