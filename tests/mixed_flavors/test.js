function test_NonMaybe() {
  ...
  42 as ?number as $NonMaybeType<mixed>; // TODO error null | undefined ~> MixedT (NonMaybe)
}

function test_function(fn: mixed) {
  if (typeof fn === "function") {
      const one = 1 as typeof fn; // TODO error number ~> MixedT (Mixed_function)
      one.apply;
  }
}
...
function test_truthy(x: mixed) {
  if (x) {
      const false_ = false as typeof x; // TODO error false ~> MixedT (Mixed_truthy)
