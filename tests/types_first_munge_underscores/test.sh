...
assert_ok "$FLOW" full-check --builtin-lib prelude .

printf "\nCheck with --munge-underscore-members flag should return one error on the _x update:\n"
assert_errors "$FLOW" full-check --builtin-lib prelude --munge-underscore-members .
...
assert_errors "$FLOW" full-check --builtin-lib prelude .
