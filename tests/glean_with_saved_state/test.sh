...
echo "Indexing 1 file + its transitive deps (a.js, b.js, re-export.js) with saved state"
"$FLOW" glean --output-dir test4 --write-root test4 --saved-state-fetcher local --saved-state-no-fallback --include-transitive-deps test.js
